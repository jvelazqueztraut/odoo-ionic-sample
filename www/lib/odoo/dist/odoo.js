...
angular.module('odoo').provider('jsonRpc', function jsonRpcProvider() {
	...
	this.odooRpc = {
		odoo_server: "http://demo.moogah.com:8071",
		uniq_id_counter: 0,
		context: {'lang': 'en_EN'},
	...
	this.$get = ["$http", "$q", "$timeout", function($http, $q, $timeout) {
		...
		*/
		odooRpc.isLoggedIn = function (force) {
			...
			var response = odooRpc.getSessionInfo().then(function (result) {
				cookies.set_sessionId(result.session_id);
				return !!(result.uid); 
			});
			...
			*/
			function preflight() {
				...
				return preflightPromise || http('/web/webclient/version_info', {}).then(function (reason) {
					console.log(reason.result.server_serie);
					console.log(reason.result.server_version);
